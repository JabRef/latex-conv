...
/// Renders a parsed SnuggleTeX token tree as Unicode plain text, driven by [ConversionTable].
///
...
public final class UnicodeEmitter {
    ...
    private UnicodeEmitter(ConversionOptions options) {
        this.options = options;
    }

    public static String emit(SnuggleSession session, ConversionOptions options) {
        ...
        return new UnicodeEmitter(options).walk(session.getParsedTokens());
    }

    private String walk(List<FlowToken> tokens) {
    ...
    private String walkSingle(FlowToken token) {
        return walk(List.of(token));
    }

    /// Emits the token at `index` (possibly consuming look-ahead siblings, e.g. a math accent's
    ...
    private int emitToken(List<FlowToken> tokens, int index, StringBuilder out) {
        ...
        return switch (token.getType()) {
            ...
            case MATH_CHARACTER -> {
                out.append(((MathCharacterToken) token).getMathCharacter().getChars());
                yield index + 1;
            }
            default -> {
                out.append(token.getSlice().extract());
                yield index + 1;
            }
        };
    }

    private void emitText(FlowToken token, StringBuilder out) {
        ...
        ComputedStyle style = token.getComputedStyle();
        ...
        switch (name) {
            ...
            case "<msupormover>", "<msubormunder>" -> {
                ...
                if (!options.convertScripts()) {
                    out.append(command.getSlice().extract());
                    return index + 1;
                }
                out.append(walkArgument(command, 0));
                out.append(scriptify(walkArgument(command, 1), superscript));
                return index + 1;
            }
            case "textsuperscript", "textsubscript" -> {
                ...
                out.append(scriptify(walkArgument(command, 0), "textsuperscript".equals(name)));
                return index + 1;
            }
            case "enquote", "mkbibquote" -> {
                out.append('“').append(walkArgument(command, 0)).append('”');
                return index + 1;
            }
            case "mkbibemph" -> {
                out.append(mapThroughStyle(walkArgument(command, 0), "mathit"));
                return index + 1;
            }
            default -> {
    ...
    private int emitGenericCommand(List<FlowToken> tokens, int index, CommandToken command, String name, StringBuilder out) {
        ...
        if (bySlice.isPresent()) {
            out.append(bySlice.get());
            return index + 1;
        }
        Optional<String> byName = ConversionTable.commandToUnicode("\\" + name);
        if (byName.isPresent()) {
            out.append(byName.get());
            return index + 1;
        }
        Map<Integer, Integer> styleMap = styleMap(name);
        if (!styleMap.isEmpty()) {
            out.append(mapThroughStyle(walkArgument(command, 0), name));
            return index + 1;
        }
        ...
        switch (options.unknownCommandPolicy()) {
            case KEEP_COMMAND -> out.append(slice);
    ...
    private void emitEnvironment(EnvironmentToken environment, StringBuilder out) {
        ...
        if ("math".equals(name) || "displaymath".equals(name)) {
            if (options.mathMode() == ConversionOptions.MathMode.PASSTHROUGH) {
                out.append(environment.getSlice().extract());
            } else {
    ...
    private int emitError(List<FlowToken> tokens, int index, StringBuilder out) {
        ...
        if (error.getError().getErrorCode() == CoreErrorCode.TTEM03) {
            ...
            if (document.charAt(slice.getEndIndex()) == '{') {
                ...
                if (close >= 0) {
                    ...
                    out.append(character).append('(').append(inner).append(')');
                    return next;
                }
            }
        ...
        switch (options.unknownCommandPolicy()) {
            case KEEP_COMMAND -> {
                out.append(document.extract(slice.getStartIndex(), end));
    ...
    private String scriptify(String script, boolean superscript) {
        StringBuilder mapped = new StringBuilder();
        boolean allMapped = true;
        for (int cp : script.codePoints().toArray()) {
    ...
    private static String mapThroughStyle(String text, String styleCommandName) {
    ...
    private static Map<Integer, Integer> styleMap(String styleCommandName) {
